...
/** Resolve the platform inventory directory, preferring the sibling repo. */
function resolveInventoryRoot(): string {
  const candidates = [
    process.env.GROWRIG_INVENTORY_DIR,
    path.resolve(repoRoot, '../growrig-platform/inventory'),
    path.resolve(repoRoot, 'source/growrig-platform/inventory'),
  ...
  throw new Error(
    `Cannot find the GrowRig inventory catalog. Set GROWRIG_INVENTORY_DIR or place growrig-platform beside this repo. Checked:\n${candidates
      .map((c) => `  - ${c}`)
      .join('\n')}`,
  );
}
